...
export default function CategoryProfiles() {
  ...
  useEffect(() => {
    if (user && params.category) {
      // Dekodiranje kategorije iz URL-a
  ...
  const fetchProfilesByCategory = async (category: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter profiles that have this category in their solutions_for array
        const filteredProfiles = allProfiles.filter((profile: CreatorProfile) => {
          console.log('Profile solutions_for:', profile.solutions_for);
          const hasCategory = profile.solutions_for && profile.solutions_for.includes(category);
          console.log('Has category:', hasCategory);
          return hasCategory && profile.full_name && profile.title;
        });
      ...
      <div className="flex-1 ml-64 relative z-10">
        ...
        <div className="bg-white/5 backdrop-blur-sm py-6">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <button
              onClick={() => router.push('/dashboard')}
              className="text-white/70 hover:text-white mb-4 flex items-center gap-2 transition"
              style={{fontFamily: 'Rockwell, serif'}}
            >
              ← Back to Dashboard
            </button>
            <h2 className="text-3xl font-bold text-white" style={{fontFamily: 'Rockwell, serif'}}>
              {categoryName}
            </h2>
            <p className="text-white/80 mt-2" style={{fontFamily: 'Rockwell, serif'}}>
              {profiles.length} {profiles.length === 1 ? 'creator' : 'creators'} offering solutions in this category
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            {profiles.length === 0 ? (
              <div className="bg-white/10 backdrop-blur-md rounded-lg p-8 text-center">
                <p className="text-white/70 text-lg" style={{fontFamily: 'Rockwell, serif'}}>
                  No creators have registered for this category yet.
                </p>
                <p className="text-white/50 mt-2" style={{fontFamily: 'Rockwell, serif'}}>
                  Be the first to offer solutions in {categoryName}!
                </p>
              </div>
            ) : (
              profiles.map((profile) => (
                <div
                  ...
                  className="bg-white/10 backdrop-blur-md rounded-lg p-6 hover:bg-white/20 transition-all cursor-pointer transform hover:scale-[1.02] duration-200"
                >
                  <div className="flex items-center gap-6">
                    {profile.avatar_url ? (
                      ...
                      <div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center">
                        <span className="text-2xl text-white font-bold" style={{fontFamily: 'Rockwell, serif'}}>
                    ...
                    <div className="flex-1">
                      <h3 className="text-2xl font-bold text-white mb-1" style={{fontFamily: 'Rockwell, serif'}}>
                        {profile.full_name}
                      </h3>
                      <p className="text-lg text-white/80" style={{fontFamily: 'Rockwell, serif'}}>
                        {profile.title}
                      </p>
                    </div>
                    ...
                    {profile.video_url && getVideoThumbnail(profile.video_url) && (
                      <div className="relative w-32 h-20 rounded-lg overflow-hidden border-2 border-white/30 flex-shrink-0">
                        <img 
                          src={getVideoThumbnail(profile.video_url) || ''} 
                          alt="Video thumbnail"
